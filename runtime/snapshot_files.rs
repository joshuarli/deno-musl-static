...
/// `(specifier, absolute source path)` form. The snapshot build script uses
/// this to embed sources that are not consumed by the startup snapshot.
#[derive(Clone, Debug)]
...
/// Construct the runtime extension graph used by both snapshot creation and
/// residual-source generation. This path only inspects extension metadata and
/// does not require a JavaScript engine snapshot.
...
pub fn collect_lazy_extension_files(
